...
**Data:** Over 55% of post-purchase survey respondents cite price as what almost stopped them, yet reviews describe Eco Satin as "amazing," "so soft," "you go to bed feeling like a queen" (Source: Surveys, 214 responses, Feb 6 - Mar 8, 2026; Reviews). The PDP scrollmap shows fewer than 15% of visitors reach the reviews where this value proof lives (Source: Heatmaps). "Eco satin" was searched 672 times in the last 30 days, and one survey respondent wrote "Eco satin sounds sus" (Source: Site Search, Surveys). Lake Pajamas features a full-width "SILKY SMOOTH DREAMMODAL" fabric story hero on every PDP (Source: Inspiration). The homepage storytelling module case study generated +$130,251/mo for a luxury pajama brand by showing craft before price (Source: Winning Tests Library).
...
**Sequencing note:** PF23 (trust badges below CTA) and PF24 (fit & size) are running on PDP. This test targets a different zone (below trust badges, above accordions). If PF23 or PF24 conclude before this test launches, incorporate their winning elements into the control.
...
**Sequencing note:** No current tests running on the homepage. This test can launch immediately.
...
**Type:** A/B test (4 variations vs. control)
**Page:** Collection page /collections/spring-pajamas-apparel (https://www.printfresh.com/collections/spring-pajamas-apparel)
...
**Sequencing note:** PF25 (3x collection grid) is running on collection pages. Confirm with the team whether PF25 targets /collections/spring-pajamas-apparel specifically. If so, delay this test until PF25 concludes or target a different collection page (e.g., /collections/pajamas at 71,452 sessions). The ATF changes in this test (hero, chips, mid-grid banner) are above and between the product grid, not modifying the grid itself, so they may coexist with PF25 if PF25 only changes grid column count. Confirm before launch.
...
**Type:** A/B test (4 variations vs. control)
**Page:** Collection page /collections/bagheera (https://www.printfresh.com/collections/bagheera)
...
**Sequencing note:** No current tests running specifically on /collections/bagheera (PF25 targets need to be confirmed). This page receives consistent paid traffic, making it a stable testing environment.
...
1. **Cart Drawer Value Reinforcement:** The cart already has a free shipping progress bar, but no "You save $X" line on discounted items, no fabric micro-badges per item, and no trust signals. Cart drawer tests win 3/3 historically. Defer until PF16 (shipping price) and PF22 (shipping threshold) conclude so the cart experience is stable.
